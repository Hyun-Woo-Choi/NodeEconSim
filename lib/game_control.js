...
const stop_room_timer = (room_number) => {
    ...
    async function processEnergyContribution(connection, recordInfo, prevGameRecord, userGameType, parameters, roomNumber, userRound, adminRound, userSubmitList, io) {
        try {
            ...
            if (!province_id) throw new Error("Invalid province_id in parameters");
    ...
    async function insertTotalEnergyRecord(userRound, roomNumber, sumEnergy, province_id, connection) {
        ...
        if (!province_id) throw new Error('Invalid province_id value');
...
async function budgetUpdateFn(record, prevRecord, adjustment, contributionDividends, sumEnergy, userRound, roomNumber, connection) {
    ...
    async function processNewBudget(connection, record_info, prev_game_record, room_number, user_round, admin_round, user_submit_list, io) {
        try {
            ...
            await Promise.all(record_info.map(async (record) => {
                const prev_record = prev_game_record.find(r => r.id === record.id);
                if (!prev_record) {
                    throw new Error(`Previous record not found for user ID: ${record.id}`);
                }
    ...
    async function processDisasterRound(connection, record_info, prev_game_record, parameters, room_number, user_round, admin_round, user_submit_list, io) {
        try {
            const total_one = record_info.reduce((acc, curr) => acc + curr.input_one, 0);
            ...
            await connection.commit(); // 모든 작업이 성공적으로 완료되면 커밋
        } catch (err) {
            await connection.rollback(); // 오류 발생 시 롤백
        ...
        if (admin_round <= 32) {
            ...
            try {
                ...
                user_submit_list.forEach(user => {
                    io.message(io.user_2_socket_id[user.id], 'change_user_status', {
                        user_id: user.id,
                        status: '5'
                    });
                });
                ...
                await connection.commit(); // 작업 성공 시 커밋
                console.log(`Game successfully ended for room ${room_number}`);
            } catch (err) {
                await connection.rollback(); // 오류 발생 시 롤백
                console.error(`Error ending game for room ${room_number}:`, err);
                throw err; // 상위 호출 함수로 에러 전달
            } finally {
                connection.release(); // 연결 해제
            }
        }
    } catch (err) {
        console.error(`Error in proceedToNextStep for room ${room_number}, admin_round ${admin_round}:`, err);
        throw err; // 상위 함수로 에러 전달
    }
}
